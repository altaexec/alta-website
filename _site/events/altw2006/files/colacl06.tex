...
\title{Instructions for COLING/ACL 2006 Proceedings}
...
\begin{abstract}
  ...
  manuscript for the proceedings of COLING/ACL 2006. The document itself
...
This document has been adapted from the instructions for EACL-06, which were in turn adapted from the instructions for ACL-05 and
EACL-03.  All these were
...
The following formatting instructions are directed to authors of papers accepted
for publication in the COLING/ACL 2006 proceedings, including the main conference, workshops, and Student Research Workshop. The conference website http://www.acl2006.org
has additional submission information for them.
All authors are required
to adhere to these specifications. Authors are required to provide a
Portable Document Format (PDF) of their
papers. The proceedings will be printed on {\bf A4 paper}. Authors from
countries in which access to word processing systems is limited should
contact the publications chair Olivia Kwong (\nobreak{rlolivia@cityu.edu.hk})  as soon as possible.
...
Unless otherwise specified, the maximum length of a
manuscript is eight ($8$) pages for main conference papers/posters and workshop papers, and six ($6$) pages for Student Research Workshop papers, printed
single-sided (see Section~\ref{sec:length} for additional information
on the maximum number of pages).

\subsection{Electronically-available resources}

This description is provided in \LaTeX2e (\nobreak{colacl06.tex}) along with the \LaTeX2e style file used to
format it (\nobreak{colacl06.sty}) and an ACL bibliography style
(\nobreak{acl.bst}); and in PDF
format (\nobreak{colacl06.pdf}). These files are all available at http://www.acl2006.org/program/style. There is also a Microsoft Word
document template (\nobreak{colacl06.dot}) available at the same
URL. We strongly recommend the use of these style files, which have
been appropriately tailored for the COLING/ACL 2006 proceedings.
...
production of your camera-ready paper, please contact the
...
initials for given names (middle initials are allowed). Avoid capitalizing last names. The
...
should be separated from the main text by a line.\footnote{Note the line
...
Unless otherwise specified, the maximum length is eight ($8$) pages for main conference papers/posters and workshop papers, and six (6) pages for Student Research Workshop papers. The page limit
should be observed strictly. All illustrations, references, and appendices must be
accommodated within these page limits, following the formatting
instructions given in the present document.
